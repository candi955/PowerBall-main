...
RedBallExcel = xlrd.open_workbook('LottoNumList25Jun1997_20Sep2020_Table_AllFloats.xlsx')
# Creating variable to convert excel file to a dataroot (using pandas)
sheets = RedBallExcel.sheets()
for sheet in sheets:
    RedBallSheetData = np.array([[sheet.cell_value(r, c) for c in range(sheet.ncols)] for r in range(sheet.nrows)])

    #  [rows:, :columns]
    # Singling out last 4 columns, all rows (weekday,month,day,year) for source data
    sources = RedBallSheetData[:,-4:]
    # Singling out the column, all rows (RedBall) as target data (row 5 to 6 really means
    # it will only print/pull from row 6)
    target = RedBallSheetData[:, 5:6]
...
# List 1 function to place list and then transfer answer to textbox, for RedBall choice
def get_WeekDayList():
...
def writeAccuracy(buttonClicks):
    if buttonClicks == 1:
        ...
        win = tk.Toplevel()
        win.wm_title("RedBall prediction page")

        popUpLabel = tk.Label(win,
                              text="Please see the accuracy percentage of the RedBall Prediction algorithm below:\n")
...
root = tk.Tk()
root.title('RedBall Prediction Page')
...
predictionLabel = Label(second_frame, text="Prediction results RedBall:")
predictionLabel.grid(row=20, column=0)
# --------------------- Dummy 1 Listbox and Textbox RedBall-------------------------------------------------------
