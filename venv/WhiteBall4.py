...
WhiteBall4Excel = xlrd.open_workbook('LottoNumList25Jun1997_20Sep2020_Table_AllFloats.xlsx')
# Creating variable to convert excel file to a dataroot (using pandas)
sheets = WhiteBall4Excel.sheets()
for sheet in sheets:
    WhiteBall4SheetData = np.array([[sheet.cell_value(r, c) for c in range(sheet.ncols)] for r in range(sheet.nrows)])

    #  [rows:, :columns]
    # Singling out last 4 columns, all rows (weekday,month,day,year) for source data
    sources = WhiteBall4SheetData[:,-4:]
    # Singling out the column, all rows (White Ball 4) as target data (row 3 to 4 really means
    # it will only print/pull from row 4)
    target = WhiteBall4SheetData[:, 3:4]
    ...
    print(X)
    print("\n\n")
    print("Target values WhiteBall 4:")
    print(y)
...
# List 1 function to place list and then transfer answer to textbox, for White Ball 4 choice
def get_WeekDayList():
...
def writeAccuracy(buttonClicks):
    if buttonClicks == 1:
        ...
        win = tk.Toplevel()
        win.wm_title("White Ball 4 prediction page")

        popUpLabel = tk.Label(win,
                              text="Please see the accuracy percentage of the WhiteBall 4 Prediction algorithm below:\n")
...
root = tk.Tk()
root.title('White Ball 4 Prediction Page')
...
predictionLabel = Label(second_frame, text="Prediction results White Ball 4:")
predictionLabel.grid(row=20, column=0)
# --------------------- Dummy 1 Listbox and Textbox White Ball 4-------------------------------------------------------
...
# List 1 White Ball 4  choice buttons
...
# List 2 White Ball 4  choice button
